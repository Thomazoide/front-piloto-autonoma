        ...
        <Navbar collapseOnSelect expand='lg' className="bg-ua-gray shadow-lg" data-bs-theme="dark" sticky="top">
            ...
            <Navbar.Collapse id='rsponsive-navbar-nav' >
                <Nav className='me-auto justify-evenly items-center align-center w-[100%]' defaultActiveKey={props.llaveActiva}>
                    <Nav.Link eventKey={"1"} accessKey={`/dashboard/${state.user?.token.split('.')[0]}`} href={`/dashboard/${state.user?.token.split('.')[0]}`} onClick={handleNavigate}>Inicio</Nav.Link>
                    <Nav.Link eventKey={"2"} accessKey={`/sedes/${state.user?.token.split('.')[0]}`} href={`/sedes/${state.user?.token.split('.')[0]}`} onClick={handleNavigate}>Sedes</Nav.Link>
                    <Nav.Link eventKey={"3"} accessKey={`/docentes/${state.user?.token.split('.')[0]}`} href={`/docentes/${state.user?.token.split('.')[0]}`} onClick={handleNavigate} >Docentes</Nav.Link>
                    <Nav.Link eventKey={"4"} accessKey={`/guardias/${state.user?.token.split('.')[0]}`} href={`/guardias/${state.user?.token.split('.')[0]}`} onClick={handleNavigate} >Guardias</Nav.Link>
